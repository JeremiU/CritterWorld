...
class Token {
     ...
     *
     * @param type The ID of the desired token type. Checks {@code type !=
     * null}.
     * @param lineNum The line number in the input file containing this token,
     *      or {code -1} for an error token. Checks {@code lineNum > 0}
     *      or {@code lineNum == -1}.
     */
    Token(TokenType type, int lineNum) {
        assert type != null : "TokenType may not be null";
    ...
    static class NumToken extends Token {
         ...
         * @param lineNum The line number in the input file containing this
         *     token. Checks {@code lineNum > 0}. Note that a {@code NumToken}
         *     is not an error token, so the line number must be positive.
         */
    ...
    static class EOFToken extends Token {
         ...
         *
         * @param sbContents contents of the string builder at the time the EOF
         *     was encountered
         * @param lineNum The line number in the input file containing this
         *     token. Checks {@code lineNum > 0}. Note that an {@code EOFToken}
         *     is not an error token, so the line number must be positive.
         */
    ...
    static class ErrorToken extends Token {
        ...
        @Override
        public String toString() {
            return String.format("Syntax error at line %d]: %s", lineNumber(),
                                 value);
        }
    }
}
